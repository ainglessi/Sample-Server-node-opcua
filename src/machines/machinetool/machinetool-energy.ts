...
class PowerSimulator {
  ...
  public tick(): number {
    ...
    const power = Math.max(
      0,
      state.mean + state.stddev * this.normalRandom(),
    );
    this.state = this.choose(
      this.model.transitionMatrix[this.state],
    );
    return power;
  }
...
export const createMachineToolEnergyLogic = async (
  ...
  setInterval(() => {
    ...
    const voltage = addressSpace?.findNode(
      `ns=${idx};i=6196`,
    ) as UAVariable;
    ...
    const current = addressSpace?.findNode(
      `ns=${idx};i=6212`,
    ) as UAVariable;
    ...
    const power = addressSpace?.findNode(
      `ns=${idx};i=6147`,
    ) as UAVariable;
